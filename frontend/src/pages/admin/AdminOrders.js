            ...
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 border-t flex justify-between">
                <div className="flex gap-3 flex-wrap">
                  ...
                  {selectedOrder.payment_status === 'paid' && !(selectedOrder.is_parent_order && ((selectedOrder.child_order_ids || []).length > 0 || (selectedOrder.vendor_count || 0) > 1)) && <a href={downloadInvoice(selectedOrder.id)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-emerald-600 hover:bg-emerald-50 rounded-lg" data-testid="admin-order-invoice-btn"><FileText size={16} />Invoice</a>}
                  {selectedOrder.payment_status === 'paid' && selectedOrder.is_parent_order && ((selectedOrder.child_order_ids || []).length > 0 || (selectedOrder.vendor_count || 0) > 1) && <span className="flex items-center gap-2 px-3 py-2 text-blue-700 bg-blue-50 border border-blue-200 rounded-lg text-xs" data-testid="admin-order-parent-invoice-note"><FileText size={14} />Invoices on each sub-order</span>}
